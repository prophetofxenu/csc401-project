...
void EncryptedServerSocketListener::start_cleanup() {
...
EncryptedServerSocketListener::EncryptedServerSocketListener(int port,
...
std::thread* EncryptedServerSocketListener::listen() {
    ...
    auto listen_thread = new std::thread([=](EncryptedServerSocketListener *t) {
        while (true) {
            int new_sock;
            ...
            EncryptedServerSocket *conn = new EncryptedServerSocket(new_sock);
...
bool EncryptedServerSocketListener::is_listening() {
    return listen_thread != nullptr;
}


std::thread* EncryptedServerSocketListener::get_listen_thread() {
    return listen_thread;
}


void EncryptedServerSocketListener::close() {
    if (listen_thread == nullptr)
        return;
    listen_thread->join();
