...
LookupRFCResponse::LookupRFCResponse(void) : code(0), host(nullptr), port(nullptr) {}


LookupRFCResponse::LookupRFCResponse(int code, std::list<std::string> host, std::list<int> port) : code(code), host(host), port(port) {}
...
std::list<std::string> LookupRFCResponse::get_host() {
    return host;
}


void LookupRFCResponse::set_host(std::list<std::string> host) {
    this->host = host;
}


std::list<std::int> LookupRFCResponse::get_port() {
    return port;
}


void LookupRFCResponse::set_length(std::list<std::int> port) {
    this->port = port;
}
